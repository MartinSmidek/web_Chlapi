...
function pid2menu($pid) { trace();
  ...
  list($cid,$mid,$ref,$mref,$type,$program,$rok)= select(
    ...
    "tx_gncase_part AS p JOIN tx_gncase AS c ON c.uid=cid LEFT JOIN menu USING (mid)","p.uid=$pid");
...
function admin_web($typ,$uid=0) { trace();
  ...
  case 'web_reconstr': // ------------------------------------------------ definice menu
    $wid= 1;
    $qry= "INSERT INTO menu (wid,mid,ref,typ,site,mref,event,nazev,next,val,elem,title) VALUES";
    $del= "\n";
    $letos= date('Y');
    $qries= array();
    ...
    query("DELETE FROM menu WHERE wid=1"); // 1=setkani.org, 2=chlapi.online
    query($qry);
    foreach($qries as $mid=>$mids) {
      query("UPDATE menu SET mid_top=$mid WHERE mid IN ($mids)");
    }
...
function tabulka($cid,$day) { trace();
  ...
  $maximum= 0;
  $tr= mysql_qry("SELECT skupina,jmeno,poradi FROM gnucast
...
//   $y= (object)array();
  switch ( $x->cmd ) {
  case 'test':   // ---------------------------------------------- test / servant
    $y->ip= ip_get();
    $y->msg= 'ok';
    break;
  
  case 'kniha': // ---------------------------------------------- clanek / servant
    global $usergroups, $totrace, $href0, $page_mref, $ezer_local;
    $y->trace= '';
    ...
    $fileadmin= $ezer_local ? "http://setkani.bean:8080/fileadmin" : "https://www.setkani.org/fileadmin";
  ...
  case 'clanek': // ---------------------------------------------- clanek / servant
    ...
    $totrace= '';
    ezer_connect('setkani');
    list($y->autor,$y->nadpis,$y->obsah,$psano,$y->crdate,$y->tstamp)= select(
      "author,title,text,FROM_UNIXTIME(date),date,tstamp",
      "setkani.tx_gncase_part",
      "uid='{$x->pid}'");
    $y->psano= sql_date1($psano);
    $y->trace= $trace;
    // překlad na globální odkazy
    $y->obsah= preg_replace("/(src|href)=(['\"])fileadmin/","$1=$2https://www.setkani.org/fileadmin",$y->obsah);
    break;
  
  case 'galerie': // ---------------------------------------------- galerie / servant
    global $usergroups, $totrace, $href0, $ezer_local;
    $totrace= '';
  ...
  case 'me_login': // ---------------------------------------------- me login
    ...
    if ( !$x->mail ) { $y->txt= "bez vyplněné mailové adresy tě nemohu přihlásit"; goto end; }
    $ok= emailIsValid($x->mail,$err);
    if ( !$ok ) { $y->txt= "'$x->mail' nevypadá jako mailová adresa ($err)."; }
    if ( $ok ) {
      // ověření adresy z Answeru
      global $totrace, $trace, $CMS;
      $totrace= 'M';
      $trace.= "start; ";
      $oldCMS= $CMS;
      $CMS= 1;
      list($ido,$jmeno,$prijmeni,$iniciace)=
        select("id_osoba,jmeno,prijmeni,iniciace","osoba",
          "deleted='' AND kontakt=1 AND FIND_IN_SET('{$x->mail}',email)",'ezer_db2');
      $user= $ido ? "$jmeno $prijmeni" : '';
      if ( !$ido ) {
        // hlášení chybného mailu v přihlášení
        ezer_connect('setkani');
        $msg= "neznámý email:$x->mail";
        query("INSERT INTO _touch (day,time,module,menu,msg)
          VALUES (CURDATE(),CURTIME(),'chlapi.online','me_login','$msg')");
      }
    }
    // pouze ověření adresy?
    ...
    ezer_connect('setkani');
    if ( !$ok ) { goto end; }
    if ( !$ido ) { $y->txt= "adresu '$x->mail' jsi IMHO nepoužil v poslední přihlášce na akci
      YMCA Setkání"; goto end; }
    // detekce PIN
    $_SESSION['ans']['tries']++;
    $pin= $x->pin;
    if ( $pin ) {
      if ( $_SESSION['ans']['tries']>1 ) {
        $y->msg= "neplatné přihlášení";
        unset($_SESSION['ans']);
        goto end;
      }
      if ( $pin==$_SESSION['ans']['pin'] ) {
        // přihlas ANS uživatele jako FE
        $uid= 999999;
        $_SESSION['web']['fe_usergroups']= $_SESSION['ans']['fe_usergroups'];
        $_SESSION['web']['fe_userlevel']= 0;
        $_SESSION['web']['fe_user']= $uid;
        $_SESSION['web']['fe_username']= "$jmeno $prijmeni";
        $y->fe_user= $uid;
        $y->be_user= 0;
      }
      else {
        $y->txt= "to je jiný PIN, než který byl poslán v posledním mailu - zkus to ještě jednou";
        goto end;
      }
    }
    else {
      // ochrana před častým posíláním mailů
      if ( isset($_SESSION['ans']['stamp']) ) {
        $min= 3;
        $now= time();
        $sec= $now - $_SESSION['ans']['stamp'];
        if ( $sec < $min*60 && $_SESSION['ans']['tries']>1 ) {
          $y->txt= "o nový PIN lze požádat nejdříve za $min minuty";
          goto end;
        }
      }
      // vytvoření PIN, zaslání mailem a zápis do osoba
      $pin= rand(1000,9999);
                                                        display("[$pin] $jmeno $prijmeni ($ido) ... ".mysql_error());
      // odeslání mailu
      $_SESSION['ans']= array();
      ...
      $_SESSION['ans']['tries']= 0;
      // zápois do db s datem
      $now= date('Y-m-d');
      query("UPDATE osoba SET web_pin=$pin,web_pin_vydan='$now' WHERE id_osoba=$ido",'setkani');
      $y->txt= "na uvedenou adresu byl odeslán mail obsahující PIN, zapiš jej do pole vedle adresy";
      $ret= mail_send('martin@smidek.eu',$x->mail,"Rozšíření přístupu na www.setkani.org",
    ...
    break;
  */
//       // zjistíme externí IP adresu
//       $ip= ip_get();
//       $y->msg.= "<br>$ip - $x->local_ip";
//       // zjistíme, zda je mail evidovaný v databázi
//       // vytvoříme jednodenní token pro počítač: datum~externí IP~interní IP
//       $token= date('Ymd')."~$ip~$x->local_ip";
//       $md5_token= md5($token);
//       $y->msg.= "<br>secret:$token $md5_token";
  ...
  case 'fe_login': // ---------------------------------------------- fe login
    ...
    list($uid,$usergroup,$userlevel,$ezer,$user)=
      select("uid,usergroup,userlevel,ezer,CONCAT(firstname,' ',name)",
        "setkani.fe_users","username='{$x->name}' AND password='{$x->pass}'");
    ...
    if ( $x->type=='be' ) {
      ...
      if ( $ezer ) {
        ...
        $x->pass= str_replace("'",'"',$x->pass);
        list($idu,$ips)=
          select("id_user,ips","setkani._user","username='{$x->name}' AND password='{$x->pass}'");
        if ( $idu==$ezer ) {
